...
class MoltenSaltPDF:
    ...
    def analyze_pdf(self):
        ...
        for ion_pair_ca_i, pdf_data_ca_i in self.ion_pairs.items():
            if pdf_data_ca_i.type == "ca":
                ...
                b_RTO = np.zeros(len(transfer_points))
                ...
                S_i = np.zeros(len(transfer_points))*(b_RTO / (1 - b_RTO))   # Initialize cumulative survival function
                ...
                S_i_y = np.zeros(len(x_range_i))#*(b_RTO[0] / (1 - b_RTO[0]))   # Initialize S_i_y with zeros
    ...
    def plot_pdf(self):
        ...
        for data in self.plot_data:
            plt.plot(data['x_range'], data['S_i'], label=f"P(r): {data['ion_pair_ca_i']}")
            ...
            #   plt.plot(data['x_range'], data['S_i'], linestyle='-.', label=f"P(r): {data['ion_pair_ca_i']}")
