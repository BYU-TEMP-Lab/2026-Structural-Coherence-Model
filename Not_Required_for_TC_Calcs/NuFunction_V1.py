...
def Nu_integral(splines, weights, pairs_with_splines, composition_str,source,weighttype,MFP_calc,num_points,x_range):
    ...
    beta_all = np.zeros(num_points)
    beta_integral = np.zeros(num_points)
    x_SCL_all = 0
    ...
    for pair, spline in splines.items():
        ...
        #max_spline = spline(x_range)

        Nu_pair[0] = np.max(spline(x_range))
        ...
        first_peak = True
        for pair, spline in splines.items():
            catan, ion1, ion2 = identify_cat_an(pair)
            ...
            y_array = spline(x_range)

            max_cat_an_y_array = np.max(cat_an_y_array)
            ...
            for i in range(len(x_range)):
                if i == 0:
                    ...
                    max_g_ideal = np.max(cat_an_y_array[0:i])

                if sum_y_arrays[i] == 0 or max_g_ideal == 0:
                    beta_pair[i] += 0
                    continue

                if cat_an_y_array[i] > cat_an_weight or first_peak == True:
                    if cation == ion1 and anion == ion2:    # If the current pair is the currently analyzed pair (i.e. K-F)
                        pass #beta_pair[i] += (1 - cat_an_y_array[i]/max_g_ideal)*(y_array[i]/sum_y_arrays[i])
                    elif cation == ion1 and cation == ion2: # If the current pair involves the current cation with itself (i.e. K-K)
                        beta_pair[i] += (1 - (cat_an_y_array[i]-y_array[i])/max_g_ideal)*(y_array[i]/sum_y_arrays[i])
                    elif (cation == ion1) ^ (cation == ion2) and catan == 2: # If the current pair involves the current cation with another cation (i.e. K-Na)
                        beta_pair[i] += (1 - (cat_an_y_array[i]-y_array[i])/max_g_ideal)*(y_array[i]/sum_y_arrays[i])
                elif cat_an_y_array[i] < cat_an_weight:
                    if cation == ion1 and anion == ion2:   # If the current pair is the currently analyzed pair (i.e. K-F)
                        beta_pair[i] += (1 - (cat_cat_y_array[i]-y_array[i])/max_g_ideal)*(y_array[i]/sum_y_arrays[i])
                    elif cation == ion1 and cation == ion2: # If the current pair involves the current cation with itself (i.e. K-K)
                        pass #beta_pair[i] += max_cat_an_y_array - y_array[i]
                    elif (cation == ion1) ^ (cation == ion2) and catan == 2: # If the current pair involves the current cation with another cation (i.e. K-Na)
                        beta_pair[i] += (1 - (cat_cat_y_array[i]-y_array[i])/max_g_ideal)*(y_array[i]/sum_y_arrays[i])

                if (np.max(cat_an_y_array[i]) - np.max(cat_an_y_array[i-1])) < 0:
                    first_peak = False
        ...
        for i in range(len(x_range)):
            ...
            Nu_pair[i] = np.exp(-beta_integral_pair[i])

        # Expectation value of Nu
        Nu_exp = np.trapz(Nu_pair*x_range,Nu_pair)

        # Find the x value corresponding to the closest y to y_avg
        index = np.argmin(np.abs(Nu_pair - Nu_exp))
        ...
        plt.plot(x_range, Nu_pair, linestyle='-.',  label=f"P(r)") # - {cat_an_pair}")

    plt.axvline(x=x_SCL_all, color='g', linestyle='--', label=f"$\lambda_{{P(r)}} = {round(x_SCL_all,2)}$")

    if MFP_calc > 0:
        plt.axvline(x=MFP_calc, color='k', linestyle='--', label=f"$\lambda_{{BC}} = {round(MFP_calc,2)}$")
    ...
    plt.close()
    print("")
    print(f"lambda_{{Nu(r)}} = {round(x_SCL_all,5)}")
