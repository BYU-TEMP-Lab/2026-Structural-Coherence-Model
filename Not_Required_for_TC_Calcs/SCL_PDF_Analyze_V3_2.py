    ...
    def analyze_pdf(self):
        ...
        for ion_pair_ca_i, pdf_data_ca_i in self.ion_pairs.items():
            if pdf_data_ca_i.type == "ca":
                ...
                y_weighted_ca_total = np.zeros(len(x_range_i))
                ...
                # Adjust minima if erratic for divalent salts
                # if spline_noise:
                #     x_0 = minima_i[0]
                #     width = 0.75*abs(peak_i[0] - minima_i[0])
                #     new_minima_value = extract_and_average(x_range_i, y_weighted_ca_i, x_0, width)
                #     minima_i = (minima_i[0], new_minima_value)
                ...
                # Bond strength/diffusion factor
                KF = (abs(peak_i[1]-minima_i[1])/peak_i[1]) 

                # Non-ideal recipient factor
                NI = (peak_i[1]/y_combined[transfer_points_indices[1]])

                # Phonon transfer factor
                PH = conc_ca_i/sum_conc_ca  # (y_weighted_ca_i[transfer_points_indices[2]]/y_weighted_ca_total[transfer_points_indices[2]])# 

                # Relative transfer efficiency of pair
                ...
                for i in range(0,len(transfer_points)):
                    ...
                    # if i % 2 != 0 and i == 1:         # First cation-to-anion transfer
                        #b_KF[i] += 1 - KF
                        #b_NI[i] += 1 - (y_weighted_ca_i[r_i]/y_combined[r_i_prev])
                        # b_SRO[i] = 1 - (y_weighted_cc_i[r_i]/y_weighted_ca_i[r_i_prev])
                    if i % 2 == 0 and i == 2:         # First anion-to-cation transfer
                        b_KF[i] += 1 - KF
                        b_NI[i] += 1 - NI#(y_weighted_cc_i[r_i]/y_combined[r_i])
                        b_PC[i] += 1 - PH
                    elif i % 2 != 0 and i > 1:         # n cation-to-anion transfer
                        b_KF[i] += 1 - KF
                        if y_combined[r_i] == 0:
                            b_NI[i] += 0
                        else:
                        ...
                        b_PC[i] += 1 - PH
                        #b_SRO[i] += 1 - (y_weighted_ca_i[r_i]/y_weighted_cc_i[r_i_prev])
                    elif i % 2 == 0 and i > 2:         # n anion-to-cation transfer
                        b_KF[i] += 1 - KF
                        if y_combined[r_i] == 0:
                            b_NI[i] += 0
                        else:
                ...
                for k in range(0,len(transfer_points_indices)):
                    beta_i_integral += beta_i[k]*(transfer_points[k] - transfer_points[k-1])
                ...
                # Append data for this pair to csv_data
                csv_data.extend([ion_pair_ca_i, x_SCL_pair, peak_i[0], peak_i[1], minima_i[0], minima_i[1]])
        ...
        print(f"lambda_BC = {round(self.gamma_bc, 5)}")
         ...
         # Write to CSV
        csv_filename = 'SCL_PDF_results_V3.csv'
        ...
        with open(csv_filename, mode='a', newline='') as file:
            ...
            if not file_exists:
                header = ['Composition', 'Pair1', 'x-SCL1', 'Peak-x1', 'Peak-y1', 'Min-x1', 'Min-y1',
    ...
    def plot_pdf(self):
        ...
        for data in self.plot_data:
            ...
            if 'ion_pair_ca_i' in data:
                ...
                plt.plot(data['x_range'], data['S_i'], label=f"S(r): {ion_pair}", color=color, linestyle='dotted',)
